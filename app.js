...
app.get('/states/', async (request, response) => {
  const getStatesQuery = `
    ...
    *
    FROM
    state`
  const allStateArray = await db.all(getStatesQuery)
  response.send(
    allStateArray.map(eachObj => convertDbObjectToResponseObject(eachObj)),
  )
})
    ...
    INSERT INTO
      district (district_name,
        ...
        VALUES(
              ...
              "${active}",
              "${deaths}")`
  const dbResponse = await db.run(addingNewDistict)
